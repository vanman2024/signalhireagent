...
class AgentCoordinator:
    ...
    def parse_tasks(self) -> List[Task]:
        ...
        # First, get ALL completed tasks (for dependency resolution)
        all_completed_pattern = r'- \[x\] (T\d+)'
        self.all_completed_tasks = set()
        for match in re.finditer(all_completed_pattern, content):
            self.all_completed_tasks.add(match.group(1))
            
        # Then get tasks with @ assignments
    ...
    def _extract_dependencies(self, task_id: str, content: str) -> List[str]:
        ...
        # Simple dependency rules based on task structure (use T001 format, not T001)
        if 14 <= task_num <= 19:  # Models depend on tests (T004-T013)
            dependencies = [f"T{i:03d}" for i in range(4, 14)]
        elif 20 <= task_num <= 23:  # Services depend on models (T014-T019)
            dependencies = [f"T{i:03d}" for i in range(14, 20)]
        elif 24 <= task_num <= 30:  # CLI depends on services (T020-T023)  
            dependencies = [f"T{i:03d}" for i in range(20, 24)]
        elif 31 <= task_num <= 34:  # Integration depends on core (T014-T030)
            dependencies = [f"T{i:03d}" for i in range(14, 31)]
        elif 35 <= task_num <= 40:  # Polish depends on implementation (T014-T034)
            dependencies = [f"T{i:03d}" for i in range(14, 35)]
        
        # Dependencies are already in correct T001 format, just filter out self
    ...
    def get_ready_tasks(self, tasks: List[Task]) -> List[Task]:
        ...
        # Use all completed tasks from the file, not just those with @ assignments
