#!/usr/bin/env python3
"""
Import already revealed contact information from SignalHire to Airtable using Person API direct retrieval.
This script uses the SignalHire Person API GET endpoint to directly retrieve contact information 
for contacts that already have revealed data, bypassing the cache system.
"""
...
async def get_signalhire_contact_direct(contact_id):
    """Get contact information from SignalHire Person API directly."""
    headers = {
    ...
    async with httpx.AsyncClient() as client:
        try:
            # Use Person API direct retrieval endpoint
...
async def main():
    """Main function to import revealed contact information using Person API direct retrieval."""
    print("🔍 IMPORTING REVEALED CONTACTS USING PERSON API DIRECT RETRIEVAL")
    print("=" * 70)
    ...
    for i, contact in enumerate(contacts, 1):
        ...
        # Get contact info from SignalHire Person API directly
        contact_data = await get_signalhire_contact_direct(contact['signalhire_id'])
