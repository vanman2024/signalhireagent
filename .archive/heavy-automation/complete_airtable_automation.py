...
class CompleteAirtableAutomation:
    ...
    def __init__(self, webhook_port: int = 8000):
        ...
        # Use environment variable for callback URL if available, otherwise localhost
        callback_base_url = os.getenv('SIGNALHIRE_CALLBACK_URL')
        if callback_base_url:
            self.webhook_url = f"{callback_base_url}/signalhire/callback"
        else:
    ...
    def _process_webhook(self, callback_data: PersonCallbackData) -> None:
        ...
        try:
            ...
            # Run async processing in new event loop since we're in a thread
            asyncio.run(self._process_webhook_async(callback_data))
    ...
    async def _process_webhook_async(self, callback_data: PersonCallbackData) -> None:
