...
async def bulk_reveal_example():
    ...
    try:
        ...
        print(f"   # Search")
        print(f'   python3 -m src.cli.main search --title "Heavy Equipment Mechanic" --location "Canada" --output prospects.json')
        print(f"   ")
        print(f"   # Start callback server (separate terminal)")
        print(f"   python3 -m src.cli.main callback-server start --port 8000")
        print(f"   ")
        print(f"   # Bulk reveal")
        print(f"   python3 -m src.cli.main reveal bulk --search-file prospects.json --callback-url http://localhost:8000/callback --output contacts.csv")
